...
// should have put the jobs in the correct order based on run time
...
int main(void)
{
  ...
  for (int i = 0; i < 1; i++)
  {
    tids[4*i] = thread_create(fool, NULL, 0);
    tids[4*i + 1] = thread_create(fool2,NULL,0);
    tids[4*i + 2] = thread_create(fool3,NULL,0);
    tids[4*i + 3] = thread_create(fool4,NULL,0);
  }
