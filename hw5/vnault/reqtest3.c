// Third required test, this program checks to make sure that Mem_Alloc
// can correctly allocate several 8-byte-aligned requests in sequence,
...
int main() {
    ...
    perform_allocation(BYTES8, 1, BYTES8_VAL_TO_WRITE1);
    perform_allocation(BYTES8, 2, BYTES8_VAL_TO_WRITE2);
    perform_allocation(BYTES16, 3, BYTES16_VAL_TO_WRITE);
    perform_allocation(BYTES32, 4, BYTES32_VAL_TO_WRITE);
    return 0;
}
