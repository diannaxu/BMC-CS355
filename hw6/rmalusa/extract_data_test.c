...
int main(int argc, char* argv[]){
  ...
  if(argc != 2){
    printf("Please enter the file name of the disk image whose data you want to extract\n");
    exit(EXIT_FAILURE); 
  }
  ...
  for(int i=0; i<num_inodes; i++){
    ...
    if(curr_inode->nlink >= 1){
      ...
      int* list_of_data_blocks = list_of_blocks(curr_inode, FALSE, &num_data_blocks_in_list);
      for(int k=0; k<num_data_blocks_in_list; k++){
        printf("%d\n", list_of_data_blocks[k]);
        // Write this block to the data file 
        // Get block address
        void* block_addr = disk_image + data_region_start + (block_size * list_of_data_blocks[k]);
        fwrite(block_addr, block_size, 1, data_out); 
      } 
