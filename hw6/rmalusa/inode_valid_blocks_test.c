...
//Run with ./inode_sanity_test [filename] (-strict) 
//Checks that each inode in a disk image has at least as many valid block indices stored in it as its size requires. With the "-strict" flag, also checks that the other block entries are 0.
...
int main(int argc, char* argv[]){
  ...
  // Iterate through the array of i-nodes, checking each one has valid addresses 
  for(int i=0; i<num_inodes; i++){
    ...
    if(curr_inode->nlink >= 1){
      ...
      for(int k=0; k<num_data_blocks_in_list; k++){
        assert(0<=list_of_data_blocks[k] && list_of_data_blocks[k] < num_datablocks); 
      } 
      free(list_of_data_blocks); 
    }
  }

  printf("Successfully asserted that each i-node has at least as many valid block addresses as its size requires; exiting\n"); 
